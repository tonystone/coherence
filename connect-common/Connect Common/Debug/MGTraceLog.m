...
static MGLogLevel     globalLogLevel;
static NSDictionary * loggedPrefixes;
static NSDictionary * loggedClasses;

@interface MGTraceLog ()
+ (MGLogLevel) logLevelForString: (NSString *) logLevelString;
+ (NSString *) stringForLogLevel: (MGLogLevel) logLevel;
@end
...
+ (MGLogLevel) logLevelForString: (NSString *) logLevelString {
    ...
    if      ([[logLevelString uppercaseString] isEqualToString: kLogLevelErrorString])    return MGLogLevelError;
    else if ([[logLevelString uppercaseString] isEqualToString: kLogLevelWarningString])  return MGLogLevelWarning;
    else if ([[logLevelString uppercaseString] isEqualToString: KLogLevelInfoString])     return MGLogLevelInfo;
    else if ([[logLevelString uppercaseString] isEqualToString: KLogLevelTrace1String])   return MGLogLevelTrace1;
    else if ([[logLevelString uppercaseString] isEqualToString: KLogLevelTrace2String])   return MGLogLevelTrace2;
    else if ([[logLevelString uppercaseString] isEqualToString: KLogLevelTrace3String])   return MGLogLevelTrace3;
    else if ([[logLevelString uppercaseString] isEqualToString: KLogLevelTrace4String])   return MGLogLevelTrace4;

    
    return MGLogLevelNone;
}

+ (NSString *) stringForLogLevel: (MGLogLevel) logLevel {
    
    switch (logLevel) {
        case MGLogLevelError:   return kLogLevelErrorString;
        case MGLogLevelWarning: return kLogLevelWarningString;
        case MGLogLevelInfo:    return KLogLevelInfoString;
        case MGLogLevelTrace1:  return KLogLevelTrace1String;
        case MGLogLevelTrace2:  return KLogLevelTrace2String;
        case MGLogLevelTrace3:  return KLogLevelTrace3String;
        case MGLogLevelTrace4:  return KLogLevelTrace4String;
...
+ (void) logMessage: (Class) callingClass classInstance: (id) classInstance selector: (SEL) selector sourceFile: (const char *) sourceFile sourceLineNumber: (int) sourceLineNumber logLevel: (MGLogLevel) level message: (NSString *) message {
    ...
    MGLogLevel currentLevel = globalLogLevel;
