...
class InMemoryDataStore : NSIncrementalStore {
    ...
    private typealias EntityCache =  [NSManagedObjectID : InMemoryReferenceObject]
    private typealias StoreCache  = [String : EntityCache]
    
    private var cache        = StoreCache()
    private var referenceIDs = [String : Int]()
    ...
    override class func identifierForNewStoreAtURL(storeURL: NSURL) -> AnyObject {
        return NSObject()
    }
    
    // Inform the store that the objects with ids in objectIDs are in use in a client NSManagedObjectContext
    override func managedObjectContextDidRegisterObjectsWithIDs(objectIDs: [NSManagedObjectID]) {
        
    }
    
    // Inform the store that the objects with ids in objectIDs are no longer in use in a client NSManagedObjectContext
    override func managedObjectContextDidUnregisterObjectsWithIDs(objectIDs: [NSManagedObjectID]) {
    ...
    override func loadMetadata() throws {
        ...
        guard let model = self.persistentStoreCoordinator?.managedObjectModel,
              let entitiesForConfiguration = model.entitiesForConfiguration(self.configurationName) else {
            ...
            if !object.objectID.temporaryID {
                ...
                if let name = object.entity.name {
                    if let permanentID = referenceIDs[name] {
                        ...
                        permanentIDs.append(self.newObjectIDForEntity(object.entity, referenceObject: permanentID))
                        //
...
extension InMemoryDataStore {
    ...
    private func referenceObjectWithObjectID(objectID: NSManagedObjectID) throws -> InMemoryReferenceObject  {
     ...
     */
    func executeSaveRequest(request: NSSaveChangesRequest, withContext context: NSManagedObjectContext?) throws -> AnyObject {
        ...
        for managedObject in request.deletedObjects ?? [] {
            if let entityName = managedObject.objectID.entity.name {
                if var entityReferenceObjects = cache[entityName] {
                    ...
                    //
                    entityReferenceObjects.removeValueForKey(managedObject.objectID)
                }
            }
        }
        ...
        if let inserted = request.insertedObjects {
            requestInsertsAndUpdates.appendContentsOf(inserted)
        }
        if let updated = request.updatedObjects {
            requestInsertsAndUpdates.appendContentsOf(updated)
        }
        
        for managedObject in requestInsertsAndUpdates {
            
            // We need a list of what relationships have changed for this object instance
            let changedRelationshipNames  = managedObject.changedValues().map({ (changed: (String, AnyObject)) -> String in
                return changed.0
            })
            ...
            if changedRelationshipNames.count > 0 {
                ...
                for relationshipName in changedRelationshipNames {
                    ...
                    if let relationshipSet = managedObject.primitiveValueForKey(relationshipName) as? Set<NSManagedObject> {
                        ...
                        if localReferenceRelationshipValue.count > 0 {
                            referenceObject.updateWithValues([relationshipName : localReferenceRelationshipValue], version: referenceObject.version)
                        } else {
                            referenceObject.updateWithValues([relationshipName : NSNull()], version: referenceObject.version)
                        }
                    } else {
                        // Single NSManagedObject
                        if let relatedManagedObject = managedObject.primitiveValueForKey(relationshipName) as? NSManagedObject {
                            
                            let relatedReferenceObject = try self.referenceObjectWithObjectID(relatedManagedObject.objectID)
                            
                            referenceObject.updateWithValues([relationshipName : relatedReferenceObject], version: referenceObject.version)
                        } else {
                            referenceObject.updateWithValues([relationshipName : NSNull()], version: referenceObject.version)
                        }
                    }
    ...
    */
    private func executeFetchRequest(request: NSFetchRequest, withContext context: NSManagedObjectContext?) throws -> AnyObject {
        ...
        if let entityName = request.entity?.name,
           let entityReferenceObjects = self.cache[entityName] {
            ...
            workingSet = entityReferenceObjects.values.filter { predicate.evaluateWithObject($0) }
...
//            workingSet = workingSet[fetchRange]
        ...
        // let fetchBachSize = request.fetchBatchSize
            ...
            case NSFetchRequestResultType.ManagedObjectResultType:   return try self.managedObjectResult(workingSet, request: request, context: context)
    ...
    private func managedObjectIDResult(workingSet: [InMemoryReferenceObject], request: NSFetchRequest, context: NSManagedObjectContext?) throws -> [NSManagedObjectID] {
     ...
     */
    private func dictionaryResult(workingSet: [InMemoryReferenceObject], request: NSFetchRequest, context: NSManagedObjectContext?) throws -> [String : AnyObject] {
        ...
        if let propertiesToFetch = request.propertiesToFetch as? [NSPropertyDescription] {
            ...
            properties.appendContentsOf(propertiesToFetch)
        ...
        for referenceObject in workingSet {
            ...
            for property in properties {
                ...
                if property is NSAttributeDescription {
                    ...
                    //
                    result[propertyName] = referenceObject.valueForKey(propertyName)

                } else if property is NSRelationshipDescription {
                    //
                    // Relationships are faulted in or null
                    //
                    if let relatedObject = referenceObject.valueForKey(propertyName) as? NSManagedObject {
     ...
     */
    private func managedObjectResult(workingSet: [InMemoryReferenceObject], request: NSFetchRequest, context: NSManagedObjectContext?) throws -> [NSManagedObjectID] {
//        
...
//        return result;
    ...
    /*
        ...
        */
    private func countResult(workingSet: [InMemoryReferenceObject], request: NSFetchRequest, context: NSManagedObjectContext?) throws -> [NSNumber] {
