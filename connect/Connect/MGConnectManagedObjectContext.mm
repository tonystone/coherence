//
//  MGConnectManagedObjectContext.m
//  Connect
//
...
@implementation MGConnectManagedObjectContext  {
    ...
    + (void) initialize {
        ...
        if (self == [MGConnectManagedObjectContext class]) {

            mainThreadSynchronizedContextCounts = [[NSMutableDictionary alloc] init];

            NSDictionary * initializationOptions = [MGConnectInitializationOptions initializationOptions: @{MGConnectMainThreadManagedObjectContextLimitOption: [NSNumber class]}];
    ...
    - (void) handlePersistentStoreDidSaveNotification:(NSNotification *) notification {
        ...
        if ([NSThread currentThread] != initThread) {
            ...
            @try {
                ...
                //
                // Note: MGConnectPersistentStoreCoordinator sends it's own notification that contains a self
