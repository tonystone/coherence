...
+ (void) addCoreDataModelExtensionMethodsForProperty: (objc_property_t) property {
    ...
    //
    NSString * getterTypes = [NSString stringWithFormat: @"%c@:", propertyType];
    NSString * setterTypes = [NSString stringWithFormat: @"v@:%c", propertyType];
    
    //
    // Add the setter and getter to
    // the NSManagedObjectModel class
    //
    class_addMethod([NSManagedObjectModel class], NSSelectorFromString(getterName), modelGetterImplementaiton, [getterTypes cStringUsingEncoding: NSUTF8StringEncoding]);
    LogTrace(1, @"Added method implementation to %@: [%@ %@]", NSStringFromClass([NSManagedObjectModel class]), getterName, getterTypes);
    
    class_addMethod([NSManagedObjectModel class], NSSelectorFromString(setterName), modelSetterImplementation, [setterTypes cStringUsingEncoding: NSUTF8StringEncoding]);
    LogTrace(1, @"Added method implementation to %@: [%@ %@]", NSStringFromClass([NSManagedObjectModel class]), setterName, setterTypes);
...
+ (void) addCoreDataEntityExtensionMethodsForProperty: (objc_property_t) property {
    ...
    //
    class_addMethod([NSEntityDescription class], NSSelectorFromString(getterName), entityGetterImplementaiton, [getterTypes cStringUsingEncoding: NSUTF8StringEncoding]);
    LogTrace(1, @"Added method implementation to %@:  [%@ %@]", NSStringFromClass([NSEntityDescription class]), getterName, getterTypes);
    
    class_addMethod([NSEntityDescription class], NSSelectorFromString(setterName), entitySetterImplementation, [setterTypes cStringUsingEncoding: NSUTF8StringEncoding]);
    LogTrace(1, @"Added method implementation to %@:  [%@ %@]", NSStringFromClass([NSEntityDescription class]), setterName, setterTypes);
}
    ...
    if (value) {
        ...
        switch (propertyType[1]) {
            ...
            case '@':   // NSObject * or id
            {
                ...
                //
                [self validateProperty: property valueClassName: value valueClass: valueClass];
...
- (void) validateProperty: (objc_property_t) property valueClassName: (NSString *) className valueClass: (Class) aValueClass {
    
    if (!aValueClass) {
        @throw [MGInitializationException exceptionWithName: @"Initialization Exception" reason: [NSString stringWithFormat: @"You specified class named \"%@\" for property %s but the class was not found.", className, property_getName(property)] userInfo: nil];
    }
...
static void _setModelUnsignedIntegerValue (NSManagedObjectModel *  self, SEL _cmd, NSUInteger newValue) {
    ...
    if (!storedValue || (storedValue && newValue != [storedValue unsignedIntegerValue])) {
        ...
        LogTrace(2, @"<%@: %p> value changed for setting <%@>, new value %lu", NSStringFromClass([self class]), self, key, newValue);
    }
}
...
static void _setEntityUnsignedIntegerValue (NSEntityDescription * self, SEL _cmd, NSUInteger newValue) {
    ...
    if (!storedValue || (storedValue && newValue != [storedValue unsignedIntegerValue])) {
        ...
        LogTrace(2, @"Entity \"%@\" setting <%@> changed, new value %lu", [self name], key, (unsigned long) newValue);
    }
}
