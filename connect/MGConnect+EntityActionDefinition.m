    ...
    for (NSString * actionName in [actionsLocations allKeys]) {
        ...
        if (actionInfo) {
            if ([actionInfo isKindOfClass: [NSString class]] ) {
                ...
                //
                //  actionInfo is of the form @{method, location}
                //
                // Example:
                //      @{@"POST": @"/servers/{serverID}/launch"}
                //
                NSString * methodString = [[actionInfo allKeys] lastObject];
                
                if (![methodString isKindOfClass: [NSString class]]) {
                    @throw [MGInitializationException exceptionWithName: @"Initialization Exception" reason: [NSString stringWithFormat: @"Invalid action method class %@ specified in %@ for action \"%@\"", NSStringFromClass([methodString class]), NSStringFromClass([self class]), actionName] userInfo: nil];
                }
                
                // Must be upper case to compare
                methodString = [methodString uppercaseString];
                if      ([methodString isEqualToString: MGWSDLHTTPExtensionMethodGET])      method = MGWSDLHTTPExtensionMethodGET;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodPOST])     method = MGWSDLHTTPExtensionMethodPOST;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodPUT])      method = MGWSDLHTTPExtensionMethodPUT;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodDELETE])   method = MGWSDLHTTPExtensionMethodDELETE;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodOPTIONS])  method = MGWSDLHTTPExtensionMethodOPTIONS;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodHEAD])     method = MGWSDLHTTPExtensionMethodHEAD;
                else if ([methodString isEqualToString: MGWSDLHTTPExtensionMethodTRACE])    method = MGWSDLHTTPExtensionMethodCONNECT;
                else {
                    @throw [MGInitializationException exceptionWithName: @"Initialization Exception" reason: [NSString stringWithFormat: @"Invalid action method %@ specified in %@ for action \"%@\", must be a valid HTTP 1.1 method.", methodString, NSStringFromClass([self class]), actionName] userInfo: nil];
                }
